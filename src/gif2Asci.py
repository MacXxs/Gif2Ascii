...
# Constants
OUTPUT_DIR = "../out/"  # Directory to store output frames
TIME_PER_FRAME = 0.065  # Time to wait per frame in seconds (15 FPS)
GRAYSCALE_GRADIENTS = ["@%#*+=-:. ",
                       "@#%WM&8B*$oahkbdpqw. ",
                       '''$@B%8&WM#*oahbdpqwmZO0QLCUYXzcvnxrjft/\|()1{}[]?-_+~<>i!lI;:,"^`'.    ''']
...
def ProcessFrame(frame:Image.Image, imgWidth:int, imgHeight:int, tileWidth:float, tileHeight:float, width:int, 
    ...
    Args:
        ...
        width (int): Width of the ASCII art output.
        scale (float): Scale factor for height.
        outPath (str): Path to save the processed frame.
    """
...
def ProcessGif(videoPath:str, width:int, scale:float, inverse:bool = False) -> None:
    """
    ...
    outPath = f"{OUTPUT_DIR}/{videoName}"
    ...
    for frame in ImageSequence.Iterator(im):
        ...
        #grayscale.save(f"{outPath}/frame_{frame.tell()}.png")

        if deltaTime < TIME_PER_FRAME:
            sleep(TIME_PER_FRAME - deltaTime)  # Sleep to maintain frame rate

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Convert a video file into ASCII art.')
    ...
    parser.add_argument('-s', '--scale', type=float, default=0.42, help='Scale factor for height (default: 0.42).')
    ...
    ProcessGif(args.video_file, args.width, args.scale, args.inverse)
